...
constexpr int AUDIO_BGM_SIZE = 1 << 4;
...
class ResourceManager {
 public:
  ...
  TTF_Font* font() const;
  const std::vector<std::shared_ptr<SDL_Texture>>& tilesetTextures() const;

  void resetWindow(SDL_Window* window);
  void resetFont(TTF_Font* font);
  void clearTilesetTextures();
  void addTilesetTexture(SDL_Texture* texture);

 private:
  std::unique_ptr<SDL_Window, decltype(&SDL_DestroyWindow)> window_{nullptr,
                                                                    SDL_DestroyWindow};
  std::unique_ptr<TTF_Font, decltype(&TTF_CloseFont)> font_{nullptr,
                                                           TTF_CloseFont};
  std::vector<std::shared_ptr<SDL_Texture>> tilesetTextures_{};
};
